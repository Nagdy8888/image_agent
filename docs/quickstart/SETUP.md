...
2. **Backend:** `pip install -r requirements.txt`. Create **.env** with `OPENAI_API_KEY`. Optional: `DATABASE_URL`, `OPENAI_MODEL`.
3. **Run backend:** `python -m src.server` → http://localhost:8000
